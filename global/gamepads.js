...
const process_buttons =(info)=>{
  ...
  if (buttons.length){ ['ok', 'yes', 'back'].map(x=>{
    if (buttons[gamepad_maps[x][0]].pressed) gamepad_maps[x][2]();
  })}
},
process_home =(info)=>{
  const {gamepad} = info, buttons = gamepad.buttons;
...
process_stick_right =(info)=>{ const {gamepad} = info;
  ...
  /*Top*/ if (gamepad.axes[3] < -0.3) scroll.scrollBy(0, size2);
  /*Bottom*/ if (gamepad.axes[3] > 0.3) scroll.scrollBy(0, size);
...
const process_buttons_anim =()=>{
    ...
    $wait(gamepad_maps.speed, ()=> requestAnimationFrame(process_buttons_anim));
},
process_home_anim =()=>{
...
process_stick_left_anim =()=>{ const now = Date.now();
    ...
    $wait(.125, ()=> requestAnimationFrame(process_stick_left_anim));
},
process_stick_right_anim =()=>{ const now = Date.now();
    Object.values(gamepads_by_index).forEach(process_stick_right);
    $wait(.001, ()=> requestAnimationFrame(process_stick_right_anim));
};
